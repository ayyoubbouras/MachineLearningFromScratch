...
def loss(n,w,X,y):
    somme=0
    for i in range(0,n):
        somme+=(y[i]-np.dot(w, X[i, :]))**2
    return somme/n  


def Adaline1(X, y, tol):
        n_samples = X.shape[0]
        ...
        w=weights
        fun = lambda w: loss(n_samples,w,X,y)
        x = np.linspace(-3,4,40)
        lines=ax.plot(x, f(x,w))
        while np.linalg.norm(nd.Gradient(fun)(w))>tol:
...
ax.set_title("Adaline", fontsize=15)
